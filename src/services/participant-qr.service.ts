...
export type ParticipantQrLookup = {
  eventId: string;
  firstName: string;
  lastName: string;
  documentId: string;
...
export async function lookupParticipantQr(eventId: string, documentId: string): Promise<ParticipantQrLookup | null> {
  const normalizedDoc = documentId.trim();
  if (!normalizedDoc) return null;

  if (!supabase && isDemoMode()) {
    const registration = mockRegistrations.find((item) => {
