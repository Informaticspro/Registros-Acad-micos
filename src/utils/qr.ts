...
export function buildParticipantQrPayload(payload: ParticipantQrPayload): string {
  return [PREFIX, payload.eventId, payload.qrToken, payload.documentId].join('|');
}

export function parseParticipantQrPayload(raw: string): ParticipantQrPayload | null {
...
export function extractQrLookupValue(raw: string): string {
  const parsed = parseParticipantQrPayload(raw);
